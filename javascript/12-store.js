...
const updateTotalePrice = () => {
    ...
    itemconainer.forEach((item) => {
        ...
        const counter = Number(
            item.getElementsByClassName("input-of-product")[0].value
        );
...
zoonBlackScreen.addEventListener("click", (eo) => {
    if (eo.target.classList.contains("btn-secondary")) {
        ...
        const NameProducteInBlackScreen =
            eo.target.parentElement.getElementsByClassName("Name-product")[0]
                .innerText;

        const allcards = document.querySelectorAll(".card");

        allcards.forEach((item) => {
            const AllNameProductInMyWep =
...
btnPrimary.forEach((item) => {
    item.addEventListener("click", (eo) => {
        ...
        const NameProducte =
            cards.getElementsByClassName("card-title")[0].innerText;
